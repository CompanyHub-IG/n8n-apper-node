...
The **Apper Trigger** node starts a workflow when a record is created or updated in a table. Configure it by selecting the **App** and **Table** to watch, and the event type you want to trigger the workflow.
...
For every operation, you pick the **App** and **Table** from searchable dropdowns (or enter their IDs directly). Field columns are loaded dynamically from your table's schema, so the input form always matches your table's actual fields and types, including text, number, boolean, date, dropdown/select.


**Note on People-type fields:** If your table has a "People" (user assignment) field, enter the user's ID as a plain string — the node automatically converts it to the format required by the Apper API. 
...
- **0.1.7** – Initial release. Supports Create, Get, Search, Update, Delete, Create multiple records, Update multiple records, and Delete multiple records operations for Apper table records.